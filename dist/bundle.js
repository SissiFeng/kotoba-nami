...
// src/interactive-deploy.ts
var WAVE = ["なみ", "うみ", "みず", "しお", "うず", "ながれ", "しぶき", "あわ", "さざなみ", "うねり", "とろ", "あらし", "なみ", "うみ", "みず", "あわ", "きば", "たき", "つなみ", "おき"];
var SKY = ["そら", "くも", "かぜ", "あお", "あさ", "ゆう", "ほし", "つき", "ひかり", "やま", "ふじ", "たか", "とお", "くも", "そら", "かぜ", "あか", "あさ", "ゆうぐれ", "あき"];
var FOAM = ["しろ", "ゆき", "きら", "ひか", "はな", "きり", "かすみ", "つゆ", "あき", "さえ", "しろ", "きら", "ほし", "つき", "ひかり"];
function pickWord(r, g, b) {
  const lum = 0.299 * r + 0.587 * g + 0.114 * b;
  if (b > r * 1.4 && lum < 130)
    return WAVE[Math.floor(Math.random() * WAVE.length)];
  if (lum > 190)
    return FOAM[Math.floor(Math.random() * FOAM.length)];
...
(async () => {
  ...
  const srcData = src.getContext("2d").getImageData(0, 0, img.width, img.height);
  ...
  const words = [];
  const baseFontSize = Math.max(7, Math.round(9 * sx));
  const lineHeight = Math.floor(baseFontSize * 1.2);
  const font = `${baseFontSize}px "Noto Sans JP","Hiragino Sans","Yu Gothic",monospace`;
  const gap = 1;
  for (let iy = 0;iy < img.height; iy += Math.round(lineHeight / sy)) {
    let ix = Math.random() * 4;
    while (ix < img.width) {
      const px = Math.min(Math.floor(ix), img.width - 1);
      const py = Math.min(Math.floor(iy + lineHeight / sy / 2), img.height - 1);
      const idx = (py * img.width + px) * 4;
      const r = srcData.data[idx], g = srcData.data[idx + 1], b = srcData.data[idx + 2];
      const lum = 0.299 * r + 0.587 * g + 0.114 * b;
      const text = pickWord(r, g, b);
      let width;
      try {
        const prep = prepareWithSegments(text, font);
        const st = measureLineStats(prep, 9999);
        width = st.maxLineWidth > 0 ? st.maxLineWidth : baseFontSize * text.length;
      } catch {
        width = baseFontSize * text.length;
      }
      const boost = 1.15;
      const er = Math.min(255, Math.round(r * boost + (lum < 80 ? 15 : 5)));
      const eg = Math.min(255, Math.round(g * boost + (lum < 80 ? 10 : 5)));
      const eb = Math.min(255, Math.round(b * boost + (lum < 80 ? 20 : 8)));
      const alpha = lum < 60 ? 0.7 : lum < 120 ? 0.85 : 0.95;
      const cx = ix * sx, cy = iy * sy;
      const wordWidth = width * sx;
      words.push({
        text,
        ox: cx,
        oy: cy,
  ...
  console.log(`言葉の波: ${words.length} words ready`);
  function drawCursor() {
    if (mx < 0)
      return;
    ctx.save();
  ...
  canvas.addEventListener("click", (e) => {
    ripples.push({ x: e.offsetX, y: e.offsetY, t: 0, maxR: 180 + Math.random() * 60 });
  });
  canvas.addEventListener("touchstart", (e) => {
    const t = e.touches[0], rect = canvas.getBoundingClientRect();
    ripples.push({ x: t.clientX - rect.left, y: t.clientY - rect.top, t: 0, maxR: 180 + Math.random() * 60 });
  });
  const RADIUS = 100, FORCE = 8, FRICTION = 0.88, SPRING = 0.06;
  function animate() {
    ...
    for (const w of words) {
      ...
      if (dist < RADIUS && dist > 0.1) {
        ...
        w.vx += dx / dist * force;
        w.vy += dy / dist * force;
        w.rotation += (dx > 0 ? 0.02 : -0.02) * (1 - dist / RADIUS);
        w.scale += (1.3 - w.scale) * 0.1;
      } else {
        w.scale += (1 - w.scale) * 0.05;
      }
      for (const rip of ripples) {
        const rdx = w.x - rip.x, rdy = w.y - rip.y;
        const rd = Math.sqrt(rdx * rdx + rdy * rdy);
        const wave = rip.maxR * (rip.t / 30);
        if (Math.abs(rd - wave) < 40 && rd > 0.1) {
          const rforce = (1 - Math.abs(rd - wave) / 40) * 8 * (1 - rip.t / 60);
          w.vx += rdx / rd * rforce;
          w.vy += rdy / rd * rforce;
        }
      }
      w.vx += (w.ox - w.x) * SPRING;
      w.vy += (w.oy - w.y) * SPRING;
      w.vx *= FRICTION;
      w.vy *= FRICTION;
      w.rotation *= 0.92;
      w.x += w.vx;
      w.y += w.vy;
      ctx.save();
      ctx.translate(w.x + w.width / 2, w.y + w.fontSize / 2);
      ctx.rotate(w.rotation);
      ctx.scale(w.scale, w.scale);
      let drawAlpha = w.alpha;
      if (dist < RADIUS * 1.5)
        drawAlpha = Math.min(1, w.alpha + 0.2 * (1 - dist / (RADIUS * 1.5)));
      ctx.font = font;
      ctx.fillStyle = `rgba(${w.r},${w.g},${w.b},${drawAlpha.toFixed(2)})`;
      ctx.fillText(w.text, -w.width / 2, -w.fontSize / 2);
      ctx.restore();
    }
    for (let i = ripples.length - 1;i >= 0; i--) {
      ...
      const wave = rip.maxR * (rip.t / 30);
      const a = 0.3 * (1 - rip.t / 60);
      ctx.beginPath();
      ctx.arc(rip.x, rip.y, wave, 0, Math.PI * 2);
