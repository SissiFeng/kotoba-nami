...
var WAVE = ["波", "海", "潮", "浪", "泡", "沫", "濤", "流", "渦", "水", "海", "波", "浪", "潮", "船", "舟", "帆", "渦", "流"];
var SKY = ["空", "雲", "風", "天", "気", "青", "嵐", "雷", "山", "岳", "峰", "嶺", "高", "遠", "朝", "夕", "暮", "暁"];
var FOAM = ["白", "雪", "光", "明", "純", "清", "輝", "花", "霧", "霞", "露", "冴"];
...
(async () => {
  ...
  const words = [];
  const baseFontSize = Math.max(16, Math.round(22 * sx));
  const lineHeight = Math.floor(baseFontSize * 1.8);
  const hGap = Math.floor(baseFontSize * 0.5);
  const font = `bold ${baseFontSize}px "Noto Sans JP","Hiragino Sans","Yu Gothic",monospace`;
  for (let iy = 0;iy < img.height; iy += Math.round(lineHeight / sy)) {
    let ix = Math.random() * hGap;
    while (ix < img.width) {
      ...
      try {
        ...
        width = st.maxLineWidth > 0 ? st.maxLineWidth : baseFontSize;
      } catch {
        width = baseFontSize;
      }
      const boost = 1.25;
      const er = Math.min(255, Math.round(r * boost + (lum < 80 ? 25 : 10)));
      const eg = Math.min(255, Math.round(g * boost + (lum < 80 ? 20 : 10)));
      const eb = Math.min(255, Math.round(b * boost + (lum < 80 ? 30 : 12)));
      const alpha = lum < 40 ? 0.6 : lum < 100 ? 0.8 : 0.95;
  ...
  const RADIUS = 120, FORCE = 10, FRICTION = 0.88, SPRING = 0.06;
  function animate() {
    ...
    for (const w of words) {
      ...
      if (dist < RADIUS && dist > 0.1) {
        ...
        w.vx += dx / dist * force;
        w.vy += dy / dist * force;
        w.rotation += (dx > 0 ? 0.03 : -0.03) * (1 - dist / RADIUS);
        w.scale += (1.4 - w.scale) * 0.1;
      } else {
      ...
      if (dist < RADIUS * 1.5)
        drawAlpha = Math.min(1, w.alpha + 0.25 * (1 - dist / (RADIUS * 1.5)));
      ctx.font = font;
