...
var WAVE = "波海潮浪泡沫濤流渦なみうみみずしおながれうず荒波大波白波波紋波濤怒濤船舟帆沈む浮かぶ逆波".match(/.{1,3}/g);
var SKY = "空雲風天気青嵐雷雨そらくもかぜあおあらし富士山岳峰嶺高遠ふじやまたかとお朝夕暮暁".match(/.{1,3}/g);
var FOAM = "白雪花明純清輝冴しろゆききらひか華霧霞露光".match(/.{1,3}/g);
...
(async () => {
  ...
  const words = [];
  const fontSize = Math.max(7, Math.round(10 * sx));
  const lineHeight = Math.floor(fontSize * 1.2);
  const font = `${fontSize}px "Noto Sans JP","Hiragino Sans","Yu Gothic",monospace`;
  for (let iy = 0;iy < img.height; iy += Math.round(lineHeight / sy)) {
    ...
    while (ix < img.width) {
      ...
      try {
        ...
        width = st.maxLineWidth > 0 ? st.maxLineWidth : fontSize * text.length;
      } catch {
  ...
  canvas.addEventListener("click", (e) => {
    ripples.push({ x: e.offsetX, y: e.offsetY, t: 0, maxR: 150 + Math.random() * 50 });
  });
  canvas.addEventListener("touchstart", (e) => {
    const t = e.touches[0], rect = canvas.getBoundingClientRect();
    ripples.push({ x: t.clientX - rect.left, y: t.clientY - rect.top, t: 0, maxR: 150 + Math.random() * 50 });
  });
  ...
  function animate() {
    ...
    for (const w of words) {
      ...
      for (const rip of ripples) {
        ...
        if (Math.abs(rd - wave) < 30 && rd > 0.1) {
          const rforce = (1 - Math.abs(rd - wave) / 30) * 6 * (1 - rip.t / 60);
          w.vx += rdx / rd * rforce;
          w.vy += rdy / rd * rforce;
        }
      }
